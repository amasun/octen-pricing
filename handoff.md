...
4. **Pay-As-You-Go 方案对比与三套架构 (Plan A vs Plan B vs Plan C)**：
   ...
   - **三张入口卡片统一 Octen Hub 规范与 Card 3 动态细节 (3 Cards Unified Hub & Card 3 Motion)**：
     ...
     - **全卡片统一背景中心聚光灯 (Unified Center Spotlight)**：以卡片 3 为视觉基准，三张卡片均垫入相同的中心绿色径向光斑（`radial-gradient(circle at 50% 50%, rgba(112, 254, 126, 0.22) 0%, rgba(3, 152, 85, 0.08) 45%, transparent 75%)`），常态 `opacity-60`，Hover 时平滑升至 `opacity-95`；

...
4. **Pay-As-You-Go 方案对比与三套架构 (Plan A vs Plan B vs Plan C)**：
   ...
   - **How Octen Works 扩展为 3 张卡片布局与去按钮化极简交互 (How Octen Works 3 Cards & No-Button Minimalist)**：
     ...
     - 布局由 2 张卡片平滑扩展为 3 张卡片：
      ...
      - **弱化浅灰【+】与【/】逻辑连接符与组团间隙对比 (Subtle Muted Symbols & Grouping Contrast)**：结构上将 Card 2 (QPS Plan) 与 Card 3 (Enterprise Plan) 划入同组嵌套容器，组内间距收窄至 gap-[4px] sm:gap-[6px] md:gap-[8px]，而 Card 1 与该组之间保持 md:gap-6 lg:gap-7 呼吸间隙，通过宏微观间隙对比刻意营造出「左侧底座 + 右侧吞吐二选一组」的清晰组团感；
...
5. **最新文案与结构对齐（2026-09-01 全量对齐 https://octen.ai/pricing）**：
   ...
   - **PRICING DETAILS 列格式统一与两端对齐 (Pricing Details Column Normalization & Space-Between Alignment)**：
    ...
    - **四大分类标题字号规范为 18px 与分类图标规范为 20×20 (Category Header Typography & Icon 20×20)**：
      ...
      - **分类矢量图标尺寸**：四大分类标题左侧的前缀 SVG 图标（`SearchCategoryIcon`、`ExtractCategoryIcon`、`EmbeddingCategoryIcon`、`ApplicationsCategoryIcon`）全量统一为 **`20×20px`**（`w-[20px] h-[20px]`），保持各视口与形态下的饱满度和高辨识度。

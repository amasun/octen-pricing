...
export default function ApiExplorer() {
  ...
  return (
    <div className="bg-white content-stretch flex flex-col items-center pb-[40px] sm:pb-[60px] pt-[50px] sm:pt-[110px] relative shrink-0 w-full max-w-[1320px] px-4 box-border" id="api-pricing">
      ...
      {/* Model Gateway Standalone Section (Arranged below Pay-As-You-Go Tabs - QPS Light Card Style) */}
      <div className="w-full mt-12 sm:mt-16 pt-8 border-t border-[#e4e4e7] flex flex-col gap-6" id="model-gateway">
        {/* Top Ecosystem Statement Banner - Matching QPS Plan Card Light / Emerald Aesthetics */}
        <div className="bg-[#f0f9f4] border border-[#bce8c9] rounded-[16px] p-6 sm:p-8 relative overflow-hidden shadow-sm">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
            <div className="flex items-start sm:items-center gap-4">
              <div className="size-[48px] rounded-[12px] bg-[#039855] text-white flex items-center justify-center shrink-0 shadow-sm">
                <svg className="size-[26px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
              </div>
              <div>
                <h3 className="font-['Fraunces',serif] font-bold text-[20px] sm:text-[24px] text-[#100F09] flex items-center gap-3 flex-wrap">
                  Unified LLM Model Gateway
                  <span className="font-['DM_Sans',sans-serif] font-semibold text-[12px] px-2.5 py-0.5 rounded-full bg-[#70FE7E] text-[#100F09]">
                    原厂计费 零溢价透传
                  </span>
                </h3>
                <p className="font-['DM_Sans',sans-serif] text-[14px] sm:text-[15px] text-[#515151] mt-1.5 leading-relaxed max-w-[850px]">
                  Octen 提供统一的大模型 Gateway 转发，方便在一个 Dashboard 中统筹搜索 API 与大模型 Token 账单，原厂计费，零溢价透传。
                </p>
              </div>
            </div>
            <div className="shrink-0 bg-white border border-[#46db98] px-4 py-2.5 rounded-[10px] text-right shadow-sm">
              <span className="block font-['DM_Sans',sans-serif] text-[11px] text-[#515151] uppercase tracking-wider font-semibold">Gateway 算力</span>
              <span className="font-['JetBrains_Mono',monospace] font-bold text-[15px] text-[#039855] whitespace-nowrap">
                起价 $0.14 / 1M tokens
              </span>
            </div>
          </div>
        ...
        {/* Brand Logo Grid Wall - Light / QPS Card Style */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <div 
              key={idx}
              className="bg-white hover:bg-[#fafafa] border border-[#e4e4e7] hover:border-[#46db98] rounded-[14px] p-5 transition-all duration-200 group flex flex-col justify-between shadow-sm hover:shadow-[0_10px_24px_rgba(3,152,85,0.12)] hover:-translate-y-[2px] cursor-pointer"
            >
              <div>
                <div className="flex items-center justify-between mb-3">
                  <span className="font-['Fraunces',serif] font-bold text-[18px] text-[#100F09] group-hover:text-[#039855] transition-colors">
                    {brand.name}
                  </span>
                  <span className="text-[11px] font-['DM_Sans',sans-serif] px-2 py-0.5 rounded-[4px] bg-[#f0f9f4] text-[#039855] border border-[#bce8c9]">
                    {brand.badge}
                  </span>
                </div>
                <p className="font-['JetBrains_Mono',monospace] text-[13px] text-[#515151] mb-4">
                  {brand.tagline}
                </p>
              </div>
              <div className="pt-3 border-t border-[#f0f0f0] flex items-center justify-between">
                <span className="font-['DM_Sans',sans-serif] text-[12px] text-[#7C7C79]">网格价格</span>
                <span className="font-['JetBrains_Mono',monospace] font-bold text-[13px] text-[#039855]">
                  {brand.price}
                </span>
              </div>
            </div>
